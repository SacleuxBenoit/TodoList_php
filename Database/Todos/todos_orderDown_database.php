...
if(isset($_GET['id_todos'])){
    ...
    $select_orderDown_todos = $bdd->prepare('SELECT order_todos FROM todos WHERE id_todos = :id_todos');
    ...
    $update_orderDown_todos = $bdd->prepare('UPDATE todos SET order_todos = :order_todos WHERE id_todos = :id_todos');
...
// else if user came from categories.php update order_categories (increase 1)
    ...
    $select_orderDown_categories = $bdd->prepare('SELECT order_categories FROM todos WHERE id_todos = :id_todos');
    ...
    $update_orderDown_categories = $bdd->prepare('UPDATE todos SET order_categories = :order_todos WHERE id_todos = :id_todos');
