...
$verify_is_check = $bdd->prepare('SELECT is_check FROM todos WHERE id_todos = :id_todos');
...
if($fetch_is_check['is_check']){
    $update_is_check = $bdd->prepare('UPDATE todos SET is_check = 0 WHERE id_todos = :id_todos');
    $update_is_check->bindParam(':id_todos', $_GET['id_todos']);
    $update_is_check->execute();
}
// if is_check is false, switch to true
else{
    $update_is_check = $bdd->prepare('UPDATE todos SET is_check = 1 WHERE id_todos = :id_todos');
    $update_is_check->bindParam(':id_todos', $_GET['id_todos']);
    $update_is_check->execute();
}
